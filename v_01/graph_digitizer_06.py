...
def optimize_one_curve_with_given_grids(
    curve_idx,
    target_center,
    *,
    img,
    ...
    best_result = optimize_curve_mask_by_feature_adjustment(
        ...
        certainty_ratio_grid=certainty_ratio_grid,
        abs_dist_grid=abs_dist_grid,
    )
    return curve_idx, best_result
...
####################################
        ...
        pass1 = Parallel(
            ...
            delayed(optimize_one_curve_with_given_grids)(
                curve_idx=i,
                target_center=target_center,
                img=img,
                centers_feat=centers_feat,
                feature_normalizer=feature_normalizer,
                origin=origin,
                ax_vec=ax_vec,
                ay_vec=ay_vec,
                ...
                ymax=ymax,
                k=k,
                inside_axes_mask=inside_axes_mask,
                axes_exclude_mask_curve=axes_exclude_mask_curve,
                legend_exclude_mask=legend_exclude_mask,
                exclude_grayscale=exclude_grayscale,
                xscale_log=xscale_log,
                yscale_log=yscale_log,
                ...
                keep_outside_axes=keep_outside_axes,
                debug=debug,
                sat_grid=fast_sat_grid,
                val_grid=fast_val_grid,
                contrast_grid=fast_contrast_grid,
                certainty_ratio_grid=fast_certainty_ratio_grid,
                abs_dist_grid=abs_dist_grid,
            )
        ...
        for curve_idx, best_result in pass1:
            if best_result is None:
                failed.append(curve_idx)
            else:
        ...
        if failed:
            ...
            pass2 = Parallel(
                ...
                delayed(optimize_one_curve_with_given_grids)(
                    curve_idx=i,
                    target_center=centers_feat[i - 1],
                    img=img,
                    centers_feat=centers_feat,
                    feature_normalizer=feature_normalizer,
                    origin=origin,
                    ...
                    keep_outside_axes=keep_outside_axes,
                    debug=debug,
                    sat_grid=full_sat_grid,
                    val_grid=full_val_grid,
                    contrast_grid=full_contrast_grid,
                    certainty_ratio_grid=full_certainty_ratio_grid,
                    abs_dist_grid=abs_dist_grid,
                )
                for i in failed
            )
        
            for curve_idx, best_result in pass2:
                if best_result is None:
...
def main_caller():
    ...
    #jobs.append(make_job_like( jobs[0], image=r"/home/romanz/code/RomanZhvanskiy/graph_digitizer/ref3figl_a.png",  xmin=0, xmax=200, ymin=0, ymax=2500, ))
